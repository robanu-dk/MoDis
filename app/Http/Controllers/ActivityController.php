...
class ActivityController extends Controller
{
    ...
    public function getDetailActivity(Request $request)
    {
        try {
            ...
            return response()->json([
                'status' => 'success',
                'data' => DB::select('SELECT a.*, ua.`id` AS `user_activity_id`, ua.`done`, uac.`id` AS `activity_coordinate_id`, u.`id` AS `user_id`, u.`name` FROM `activities` a INNER JOIN `user_activities` ua ON a.`id` = ua.`id_activity` LEFT JOIN `user_activity_coordinates` uac ON ua.`id` = uac.`id_user_activity` LEFT JOIN `users` u ON u.`id` = ua.`id_user` WHERE a.`id` = ? AND ua.`id_user` = ?', [$activity->id, $user->id])[0],
            ], 200);
        } catch (\Throwable $th) {
            return response()->json([
                'status' => 'error',
    ...
    public function deleteActivity(Request $request)
    {
        ...
        try {
            ...
            if ($user->role) {
                DB::delete('DELETE FROM `user_activities` ua WHERE ua.`id_activity` = ?', [$activity->id]);
            } else {
                DB::delete('DELETE FROM `user_activities` ua WHERE ua.`id_user` = ? AND ua.`id_activity` = ?', [$user->id, $activity->id]);
            }
